    ...
    <div className="space-y-10 animate-fade-in">
      ...
      <div className="flex items-center gap-3 bg-slate-100 p-4 rounded-2xl border border-slate-200">
          ...
          <div className="flex-grow">
              <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">Active Technology</p>
      ...
      <div className="space-y-4">
          ...
          <div className="bg-slate-100/30 border-2 border-dashed border-blue-200 rounded-3xl p-8 text-center cursor-pointer hover:border-red-400 hover:bg-slate-100 transition-all group">
              <input type="file" id="file-upload" multiple accept="image/*" onChange={handleFileChange} className="sr-only" />
              <label htmlFor="file-upload" className="cursor-pointer block">
                  <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-md group-hover:scale-110 transition-transform border border-slate-100">
                      <svg className="w-7 h-7 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"/></svg>
                  </div>
                  <p className="text-[13px] font-black text-slate-900">Kéo thả ảnh hoặc <span className="text-blue-600 underline">chọn tệp</span></p>
          ...
          <div className="grid grid-cols-1 gap-4">
              {formState.files.map(file => (
                  <div key={file.id} className={`flex flex-col bg-white p-4 rounded-2xl border transition-all ${file.role === 'product' ? 'border-blue-200 shadow-md ring-1 ring-slate-100' : 'border-slate-200 shadow-sm'}`}>
                      <div className="flex items-center gap-4">
                          ...
                          <div className="flex-grow space-y-2">
                              <select value={file.role} onChange={(e) => handleRoleChange(file.id, e.target.value as AdImageRole)}
                                  className="w-full bg-slate-100 border-none text-[11px] font-black uppercase rounded-lg p-2 focus:ring-2 focus:ring-blue-200 text-slate-900">
                                  {roleOptions.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
                              </select>
                              <div className="flex gap-2">
                                  <button onClick={() => handleProcessImage(file.id, 'remove_bg')} disabled={file.isProcessing}
                                    className="flex-grow py-1.5 px-3 bg-slate-100 text-blue-700 text-[9px] font-black uppercase rounded-lg hover:bg-blue-600 hover:text-white transition-all disabled:opacity-50">
                                      ✂️ Tách nền
                                  </button>
                                  <button onClick={() => handleProcessImage(file.id, 'retouch')} disabled={file.isProcessing}
                                    className="flex-grow py-1.5 px-3 bg-indigo-50 text-indigo-700 text-[9px] font-black uppercase rounded-lg hover:bg-indigo-600 hover:text-white transition-all disabled:opacity-50">
                                      ✨ Retouch
                                  </button>
                              </div>
                          </div>
                          <button onClick={() => handleRemoveFile(file.id)} className="p-2 text-slate-300 hover:text-blue-600 transition-all self-start">
      ...
      <div className="bg-slate-100/40 p-6 lg:p-8 rounded-[2rem] border border-slate-200 shadow-inner space-y-6">
        <label className="text-[12px] font-black text-blue-800 uppercase tracking-widest block">
        ...
        <div className="relative">
            <input type="text" value={suggestionQuery} onChange={e => setSuggestionQuery(e.target.value)} onKeyPress={e => e.key === 'Enter' && handleGetSuggestions()}
                className="w-full bg-white border-2 border-slate-100 rounded-2xl p-4 pr-16 text-[14px] font-black text-slate-900 placeholder:text-slate-300 shadow-sm focus:border-red-400 transition-all"
            ...
            <button onClick={handleGetSuggestions} disabled={isSuggesting || !suggestionQuery.trim()}
                className="absolute right-2 top-2 w-11 h-11 bg-blue-600 rounded-xl flex items-center justify-center text-white shadow-lg disabled:opacity-30">
            ...
            <div className="grid grid-cols-1 gap-3">
                {suggestions.map((s, idx) => (
                    <button key={idx} onClick={() => handleUseSuggestion(s.prompt)}
                        className="text-left p-4 bg-white border border-slate-100 rounded-2xl hover:border-blue-500 hover:shadow-lg transition-all flex items-center gap-4">
                        <div className="w-8 h-8 rounded-lg bg-slate-100 flex items-center justify-center text-blue-600 text-xs font-black shrink-0">{idx + 1}</div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        <div className="space-y-3">
            ...
            <textarea name="prompt" rows={5} value={formState.prompt} onChange={handlePromptChange} 
                className="w-full bg-white border border-slate-200 rounded-2xl p-5 text-[14px] font-black text-slate-900 placeholder:text-slate-300 focus:border-red-400 transition-all resize-none shadow-inner border-l-4 border-l-blue-600" 
            ...
            <div className="flex gap-3">
                {[1, 3, 5].map(v => (
                    <button type="button" key={v} onClick={() => handleVariationsChange(v as 1 | 3 | 5)}
                        className={`flex-grow py-3 rounded-xl text-[11px] font-black transition-all border-2 ${formState.variations === v ? 'bg-blue-600 text-white border-blue-600 shadow-lg scale-105' : 'bg-white border-slate-100 text-slate-500 hover:border-blue-200'}`}>
                        {v} BẢN VẼ
                    </button>
                ))}
            </div>
